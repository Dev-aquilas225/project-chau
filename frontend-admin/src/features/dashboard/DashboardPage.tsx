...
function StatCard({ icon: Icon, label, value, accent }: { icon: any; label: string; value: string; accent?: string }) {
  return (
    <div className="card flex items-center gap-4">
      <div className={cn('rounded-lg p-3', accent ?? 'bg-gray-100')}><Icon className="h-6 w-6" /></div>
      <div>
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard icon={DollarSign} label="Chiffre d'affaires" value={formatPrice(revenue)} accent="bg-green-100" />
        <StatCard icon={ShoppingCart} label="Commandes" value={String(orders.length)} accent="bg-blue-100" />
            ...
            {byStatus.map((b) => (
              <div key={b.status} className="flex items-center gap-3 text-sm">
                ...
                <div className="h-3 flex-1 rounded-full bg-gray-100">
                  <div className="h-3 rounded-full bg-ink" style={{ width: `${(b.count / maxCount) * 100}%` }} />
                </div>
              ...
              {recent.map((o) => (
                <li key={o.id} className="flex items-center justify-between py-2 text-sm">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-muted">{ORDER_STATUS_LABEL[o.status]}</p>
                  </div>
                </li>
              ))}
            </ul>
